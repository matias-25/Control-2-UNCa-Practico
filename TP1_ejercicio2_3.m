close all; clear;
T = readtable('Curvas_Medidas_Motor.xls');
A = table2array(T);
...
StepAmplitude=12;
temp_co=3658;
t_s=t(temp_co:22990)-t(temp_co); %dezplazo el eje t
y=wr(temp_co:22990); %se analiza la seccion del Escalon positivo 
t0=t_s;
t_inic=0.0207-t(temp_co); %tiempo inicial 
...
StepTL=-7.5e-2;
temp_co_TL=6325;
t_s_TL=t(temp_co_TL:22990)-t(temp_co_TL); %dezplazo el eje t
y_TL=wr(temp_co_TL:22990); %se analiza la seccion del Escalon positivo 
t0_TL=t_s_TL;
t_inic_TL=0.1007-t(temp_co_TL);  %tiempo inicial 
